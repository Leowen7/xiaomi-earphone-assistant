...
def _format_contexts(contexts: list[dict[str, Any]]) -> str:
    """把任务17返回的说明书片段整理成提示词文本。"""
    if not contexts:
        raise LLMServiceError("没有可供大模型参考的说明书片段")

    max_chars = int(os.getenv("LLM_MAX_CONTEXT_CHARS", "12000"))
    blocks: list[str] = []

    for index, item in enumerate(contexts, start=1):
        source_file = item.get("source_file") or "未知文件"
        page = item.get("page")
    ...
    if not context_text:
        raise LLMServiceError("说明书片段中没有有效正文")
...
def generate_manual_answer(
    ...
    if not cleaned_question:
        raise LLMServiceError("用户问题不能为空")

    context_text = _format_contexts(contexts)
    model = os.getenv("GEMINI_MODEL", "gemini-3.5-flash").strip()
    product_text = product_name or "当前所选耳机"

    prompt = f"""
你是“小米耳机智能选购与客服助手”中的说明书问答模块。
...
""".strip()
    ...
    try:
        ...
        raise LLMServiceError(f"Gemini 调用失败：{exc}") from exc

    answer = (response.text or "").strip()

    if not answer:
        raise LLMServiceError("Gemini 没有返回有效文本")
